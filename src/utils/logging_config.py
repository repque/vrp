...
class VRPFormatter(logging.Formatter):
    ...
    def __init__(self):
        ...
        self.formats = {
            ...
            logging.WARNING: "[%(asctime)s] [%(name)s] [WARNING] [%(funcName)s:%(lineno)d] %(message)s",
            logging.ERROR: "[%(asctime)s] [%(name)s] [ERROR] [%(funcName)s:%(lineno)d] %(message)s",
            logging.CRITICAL: "[%(asctime)s] [%(name)s] [CRITICAL] [%(funcName)s:%(lineno)d] %(message)s"
        }
...
def setup_logging(config: VRPTradingConfig) -> VRPLogger:
    """
